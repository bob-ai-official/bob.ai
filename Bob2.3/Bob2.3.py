...
# ============================================================
#  CHATBOT BOB 2.3  –  by X
#  Zmiany względem 2.2:
#   - NOWE: /motyw – gotowe zestawy kolorów (matrix, ocean, sunset, blood, candy, snow)
#   - NOWE: /losuj a,b,c – losuje element z podanej listy
#   - NOWE: /anagram słowo – miesza litery słowa
#   - NOWE: /stoper – stoper start/stop w konsoli
#   - NOWE: /fiszka – tryb nauki ze słownika PL<->EN
#   - NOWE: /lotto – losuje 6 liczb z 49 jak w Lotto
#   - NOWE: /cytat – losowy cytat motywacyjny (PL+EN)
# ============================================================
...
bob_fakty_pl = [
    ...
    "Mój kod rósł od wersji 1.8 aż do 2.3 - to znaczy, że mój twórca dużo pracuje!",
    ...
    "W wersji 2.3 dostałem motywy (/motyw), lotto (/lotto), cytaty (/cytat), fiszki (/fiszka), anagram (/anagram), stoper (/stoper) i losowanie (/losuj)!",
]

bob_fakty_en = [
    "I'm written in Python - a language named after Monty Python, not the snake!",
    "My code grew from version 1.8 all the way to 2.3 - my creator works hard!",
    ...
    "In version 2.3 I got themes (/motyw), lotto (/lotto), quotes (/cytat), flashcards (/fiszka), anagram (/anagram), stopwatch (/stoper) and picker (/losuj)!",
]
...
def fiszka():
    ...
    if jezyk == "pl":
        ...
        historia.append(f"Ty: {odp}")
        if odp == slowo_en:
            print(f"Program: Brawo! '{slowo_pl}' = '{slowo_en}' ✓")
        else:
        ...
        historia.append(f"Ty: {odp}")
        if odp == slowo_pl:
            print(f"Program: Correct! '{slowo_en}' = '{slowo_pl}' ✓")
        else:
...
print("/fiszka              - nauka słówek PL<->EN")
...
while True:
    ...
    if pytanie == "/help":
        ...
        print("/motyw matrix|ocean|sunset|blood|candy|snow")
        print("/teach pytanie | odpowiedz")
        print("/history  /clear  /imgs  /stats")
        print("/calc  /czas  /quiz  /fiszka  /zgadnij  /haslo")
        print("/tlumacz  /rzut [N]  /odliczaj N  /memo")
        print("/lotto  /losuj a,b,c  /anagram słowo  /stoper  /cytat")
        print("/bob")
        continue
